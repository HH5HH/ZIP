...
test("background removes legacy Slack OAuth and popup-login handlers", () => {
  ...
  assert.match(source, /const allowBotDelivery = body\.allowBotDelivery === true;/);
  assert.match(source, /const tokenAttempts = \[];/);
  assert.match(source, /const pushTokenAttempt = \(token, mode\) => \{/);
  assert.match(source, /for \(let i = 0; i < userCandidates\.length; i \+= 1\) \{/);
  assert.match(source, /if \(allowBotDelivery\) \{/);
  assert.match(source, /for \(let i = 0; i < botCandidates\.length; i \+= 1\) \{/);
  ...
  assert.match(source, /delivery_mode:\s*isBotToken \? "bot_direct_channel" : "user_direct_channel"/);
