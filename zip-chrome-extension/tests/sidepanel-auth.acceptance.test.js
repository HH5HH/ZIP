    ...
    fetch: ({ url, init }) => {
      ...
      if (url.endsWith("/api/conversations.open") && authorization === bearer(TOKENS.directUser)) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: false, error: "enterprise_is_restricted" }),
          text: async () => ""
        });
      }
...
test("ZIP_SLACK_API_SEND_TO_SELF resolves self bot delivery from the live user token when the cached user id is stale", async () => {
    ...
    fetch: ({ url, init }) => {
      ...
      if (url.endsWith("/api/conversations.open") && authorization === bearer(TOKENS.directBot)) {
        ...
        if (targetUserId === "UALICE123") {
          return Promise.resolve({
            ok: true,
            status: 200,
            json: async () => ({ ok: true, channel: { id: "DBOTSELF1" } }),
            text: async () => ""
          });
        }
      }
      if (url.endsWith("/api/chat.postMessage") && authorization === bearer(TOKENS.directBot)) {
        ...
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: true, channel: "DBOTSELF1", ts: "1711111111.000500" }),
          text: async () => ""
        });
      }
      return Promise.resolve({
  ...
  assert.equal(response && response.ok, true);
  assert.equal(String(response && response.delivery_mode || ""), "bot_direct_channel");
  assert.equal(response && response.user_id, "UALICE123");
  assert.equal(seenAuthorizations.includes(bearer(TOKENS.directUser)), true);
  assert.equal(seenAuthorizations.includes(bearer(TOKENS.directBot)), true);
...
test("ZIP_SLACK_API_SEND_TO_SELF falls back to user direct delivery when bot-preferred self DM returns user_not_found", async () => {
  ...
  const harness = createChromeHarness({
    ...
    fetch: ({ url, init }) => {
      ...
      if (url.endsWith("/api/conversations.open") && authorization === bearer(TOKENS.directBot)) {
        seenConversationUsers.push("bot:" + String(params.get("users") || ""));
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: false, error: "user_not_found" }),
      ...
      if (url.endsWith("/api/conversations.open") && authorization === bearer(TOKENS.directUser)) {
        seenConversationUsers.push("user:" + String(params.get("users") || ""));
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: true, channel: { id: "DUSERSELF1" } }),
          text: async () => ""
        });
      }
      if (url.endsWith("/api/chat.postMessage") && authorization === bearer(TOKENS.directUser)) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: true, channel: "DUSERSELF1", ts: "1711111111.000700" }),
          text: async () => ""
        });
      }
      return Promise.resolve({
  ...
  assert.equal(seenAuthorizations.includes(bearer(TOKENS.directUser)), true);
  assert.equal(seenAuthorizations.includes(bearer(TOKENS.directBot)), true);
  assert.deepEqual(seenConversationUsers, ["bot:UALICE123", "user:UALICE123"]);
});
...
test("ZIP_SLACK_API_AUTH_TEST accepts a cached PASS-TRANSITION self alias as the expected Slack user", async () => {
  const harness = createChromeHarness({
    zendeskTabs: [],
    storageSeed: {
      zip_pass_transition_recipients: [
...
test("ZIP_SLACK_API_SEND_TO_SELF resolves user self delivery through the cached PASS-TRANSITION self alias", async () => {
  const seenConversationUsers = [];
  const seenAuthorizations = [];
  const harness = createChromeHarness({
    zendeskTabs: [],
    ...
    fetch: ({ url, init }) => {
      ...
      if (url.endsWith("/api/conversations.open") && authorization === bearer(TOKENS.directUser)) {
        ...
        if (targetUserId === "WSELFALIAS1") {
          return Promise.resolve({
            ok: true,
            status: 200,
            json: async () => ({ ok: true, channel: { id: "DUSERSELFALIAS1" } }),
            text: async () => ""
          });
        }
        if (targetUserId === "UALICE123") {
      ...
      if (url.endsWith("/api/chat.postMessage") && authorization === bearer(TOKENS.directUser)) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: true, channel: "DUSERSELFALIAS1", ts: "1711111111.001000" }),
          text: async () => ""
        });
      }
      return Promise.resolve({
  ...
  assert.equal(String(response && response.user_id || ""), "WSELFALIAS1");
  assert.equal(seenAuthorizations.includes(bearer(TOKENS.directUser)), true);
  assert.deepEqual(seenConversationUsers, ["WSELFALIAS1"]);
});

test("ZIP_SLACK_API_SEND_TO_SELF accepts a cached PASS-TRANSITION self alias as the author identity", async () => {
  const seenConversationUsers = [];
  const harness = createChromeHarness({
    zendeskTabs: [],
    storageSeed: {
    ...
    fetch: ({ url, init }) => {
      ...
      if (url.endsWith("/api/chat.postMessage") && authorization === bearer(TOKENS.directUser)) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: async () => ({ ok: true, channel: "DUSERSELFALIAS1", ts: "1711111111.001100" }),
          text: async () => ""
        });
      }
      return Promise.resolve({
